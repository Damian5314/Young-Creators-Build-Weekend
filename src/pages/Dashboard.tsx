...
import { Store, Plus, Loader2, ArrowLeft, Edit, Trash2, Video, X, Eye, Heart } from 'lucide-react';
...
function RestaurantCard({ restaurant, onEdit, onDelete, onManageVideos }: {
...
function RestaurantFormModal({
  isOpen, onClose, onSave, editingRestaurant, formData, setFormData, saving
}: {
  isOpen: boolean;
  onClose: () => void;
  onSave: () => void;
          ...
          <div className="min-h-screen flex items-start justify-center p-4 py-12">
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <Input placeholder="Opening hours (e.g. 10:00 - 22:00)" value={formData.opening_hours} onChange={(e) => setFormData({ ...formData, opening_hours: e.target.value })} />
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    {[1, 2, 3, 4].map((level) => (
                      <Button key={level} variant={formData.price_level === level ? 'default' : 'secondary'} size="sm" onClick={() => setFormData({ ...formData, price_level: level })}>
                        {'€'.repeat(level)}
                      </Button>
                    ))}
                  </div>
                ...
                <button
                  ...
                  className={`w-full p-3 rounded-xl border-2 flex items-center gap-3 transition-all ${formData.halal ? 'border-primary bg-primary/10' : 'border-secondary'}`}
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {CUISINE_OPTIONS.map((cuisine) => (
                      <button
                        ...
                        className={`px-3 py-1 rounded-full text-sm font-medium transition-all ${formData.cuisine_types.includes(cuisine) ? 'bg-primary text-primary-foreground' : 'bg-secondary'}`}
                      >
                        {cuisine}
                      </button>
                    ))}
...
function VideoManagementModal({
  isOpen, onClose, restaurant, videos, onAddVideo, onDeleteVideo, saving
}: {
  isOpen: boolean;
  onClose: () => void;
  restaurant: Restaurant;
  ...
  const [videoForm, setVideoForm] = useState({ title: '', description: '', video_url: '', thumbnail_url: '' });
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-background">
          <div className="h-full flex flex-col">
            <div className="flex items-center gap-4 p-4 border-b border-border">
              ...
              <Button size="sm" onClick={() => setShowVideoForm(true)}><Plus className="h-4 w-4 mr-1" />Add Video</Button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {videos.length === 0 ? (
                ...
                <div className="space-y-3">
                  {videos.map((video) => (
                    <div key={video.id} className="flex items-center gap-3 p-3 bg-card rounded-xl border border-border">
                      <div className="h-16 w-24 rounded-lg bg-secondary overflow-hidden flex-shrink-0">
                        {video.thumbnail_url ? (
                          ...
                          <div className="h-full w-full flex items-center justify-center"><Video className="h-6 w-6 text-muted-foreground" /></div>
                        )}
                      </div>
                      <div className="flex-1">
                        <h3 className="font-semibold">{video.title}</h3>
                        <div className="flex items-center gap-4 text-sm text-muted-foreground">
                          <span className="flex items-center gap-1"><Eye className="h-4 w-4" />{video.view_count}</span>
                          <span className="flex items-center gap-1"><Heart className="h-4 w-4" />{video.like_count}</span>
                        </div>
                      </div>
                      <Button variant="ghost" size="icon" onClick={() => onDeleteVideo(video.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button>
                    </div>
                  ))}
                </div>
              )}
          ...
          <AnimatePresence>
            {showVideoForm && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-60 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm" onClick={() => setShowVideoForm(false)}>
                <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="w-full max-w-md rounded-3xl bg-card p-6 shadow-elevated" onClick={(e) => e.stopPropagation()}>
                  ...
                  <div className="space-y-4">
                    ...
                    <Input placeholder="Video URL" value={videoForm.video_url} onChange={(e) => setVideoForm({ ...videoForm, video_url: e.target.value })} />
                    <Input placeholder="Thumbnail URL" value={videoForm.thumbnail_url} onChange={(e) => setVideoForm({ ...videoForm, thumbnail_url: e.target.value })} />
                    <div className="flex gap-3">
                      <Button variant="secondary" onClick={() => setShowVideoForm(false)} className="flex-1">Cancel</Button>
                      <Button onClick={handleSubmit} disabled={!videoForm.title || !videoForm.video_url || saving} className="flex-1">
...
export default function OwnerDashboard() {
  ...
  const fetchVideos = async (restaurantId: string) => {
    const { data } = await supabase.from('videos').select('*').eq('restaurant_id', restaurantId);
  ...
  const saveRestaurant = async () => {
    ...
    if (editingRestaurant) {
      const { error } = await supabase.from('restaurants').update(restaurantForm).eq('id', editingRestaurant.id);
      if (error) toast.error('Failed to update restaurant');
      else { toast.success('Restaurant updated!'); fetchRestaurants(); }
    } else {
      const { error } = await supabase.from('restaurants').insert({ ...restaurantForm, owner_id: user.id });
      if (error) toast.error('Failed to create restaurant');
      else { toast.success('Restaurant created!'); fetchRestaurants(); }
    }
  ...
  const resetRestaurantForm = () => {
    setRestaurantForm({
      name: '', description: '', address: '', city: 'Rotterdam',
      latitude: 51.9225, longitude: 4.47917, cuisine_types: [],
      halal: false, price_level: 2, opening_hours: '', image_url: '',
    });
  };
  ...
  return (
    <div className="min-h-screen bg-background p-4">
      <div className="flex items-center gap-4 pt-4 pb-6">
        ...
        <Button onClick={() => setShowRestaurantForm(true)}><Plus className="h-4 w-4 mr-1" />Add Restaurant</Button>
      </div>
      ...
      {restaurants.length === 0 ? (
        ...
        <div className="space-y-4">
          {restaurants.map((restaurant) => (
            <RestaurantCard key={restaurant.id} restaurant={restaurant} onEdit={openEditRestaurant} onDelete={deleteRestaurant} onManageVideos={openRestaurantVideos} />
          ))}
        </div>
      )}
      ...
      <RestaurantFormModal
        ...
        onClose={() => { setShowRestaurantForm(false); setEditingRestaurant(null); resetRestaurantForm(); }}
      ...
      {selectedRestaurant && (
        <VideoManagementModal
          ...
          onAddVideo={async (video) => {
            setSaving(true);
            const { error } = await supabase.from('videos').insert({ ...video, restaurant_id: selectedRestaurant.id, tags: [] });
            if (error) toast.error('Failed to add video');
            else { toast.success('Video added!'); fetchVideos(selectedRestaurant.id); }
            setSaving(false);
          }}
